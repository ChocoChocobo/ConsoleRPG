...
void main()
{
	...
	for (int enemyCount = 0; enemyCount < enemyWave.size(); enemyCount++)
	{
		...
		switch (battleContext.GetBattleStateEnum())
		{
		case PlayerTurnEnum:
			player.PrintStatus();
			PAUSE_1_SECONDS;

			if (enemyWave[enemyCount].minion != nullptr && enemyWave[enemyCount].minionSpawned && battleContext.GetBattleStateEnum() == PlayerTurnEnum)
			{
				enemyWave[enemyCount].minion->PrintStatus();
				PAUSE_1_SECONDS;
				userChoice = battleContext.RequestAction(player, *enemyWave[enemyCount].minion);
			}
			else if (battleContext.GetBattleStateEnum() == PlayerTurnEnum)
			{
